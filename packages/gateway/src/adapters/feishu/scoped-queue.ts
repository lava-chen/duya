 ...
 * 同一 scope 内的消息在 quiet window 内合并成一个 batch 一起处理。
 *
 * 关键语义(参考 Proma feishu-bridge 的 ScopedQueue):
 ...
 * - cancel: 命令直通场景——丢弃此前累积的普通消息,返回当前累积
 *
 * 这个模式解决两个真实痛点:
...
export class ScopedQueue<T> {
  ...
  /** 所有 scope 累积的消息总数(诊断用)。 */
  totalPending(): number {
    let n = 0;
    for (const entry of this.map.values()) {
      n += entry.messages.length;
    }
    return n;
  }

  private armTimer(scope: string): ReturnType<typeof setTimeout> {
  ...
  private flush(scope: string): void {
    ...
    try {
      ...
      // 业务侧 handler 抛错不应影响队列状态;但要记录以便排查
      // eslint-disable-next-line no-console
      console.error('[Feishu ScopedQueue] flush handler threw', { scope, err });
    }
  }
}
