 ...
 *
 * Depends on `@duya/agent`'s prompt subsystem at runtime: this class
 * extends `PromptSystem` and uses `createPromptCache`,
 * `asSystemPrompt`, and `SYSTEM_PROMPT_DYNAMIC_BOUNDARY` from the
 * agent. The host's prompt registry is what owns the actual
 * registration — see `registerConductor()` in `@duya/conductor`.
 */

import { createPromptCache } from '@duya/agent/prompts';
import { asSystemPrompt, SYSTEM_PROMPT_DYNAMIC_BOUNDARY } from '@duya/agent/prompts/types';
import type { PromptContext, PromptSection, SystemPrompt, PromptBuildContextOptions } from '@duya/agent/prompts/types';
import { PromptSystem } from '@duya/agent/prompts';
import { isSectionEnabled } from '@duya/agent/prompts/modes';
import type { PromptProfile } from '@duya/agent/prompts/modes/types';
import { getShellForPrompt } from '@duya/agent/utils/shellDetector';
...
import { buildConductorCanvasSection } from './canvasSection.js';
...
export class ConductorPromptSystem extends PromptSystem {
  ...
  override getDynamicSections(
    ...
    return [
      kept('conductorCanvas', () => buildConductorCanvasSection(context), 'Canvas workspace context'),
  ...
  override buildContext(options: PromptBuildContextOptions): PromptContext {
    const workingDirectory =
      options.workingDirectory !== undefined && options.workingDirectory !== null
