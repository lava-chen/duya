...
export function TaskListWidget({ data, onChange, readOnly }: WidgetComponentProps) {
  ...
  const persistedNewTaskText = (data._newTaskText as string) || "";
  const [newTaskText, setNewTaskText] = useState(persistedNewTaskText);

  useEffect(() => {
    setNewTaskText(persistedNewTaskText);
  }, [persistedNewTaskText]);
  ...
  return (
    ...
    >
      ...
      <div
        className="flex flex-col gap-1.5 flex-1 min-h-0 overflow-y-auto"
