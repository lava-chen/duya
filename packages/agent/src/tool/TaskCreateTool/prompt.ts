/**
 * TaskCreateTool Prompt - Original text from claude-code-haha
 */
...
After creating tasks, use \`TaskUpdate\` to set up dependencies (\`blocks\`/\`blockedBy\`) if needed. When an agent is ready to work on a task, it should \`TaskUpdate\` the task's \`owner\` to its own agent ID and set \`status\` to \`in_progress\`. Check first that the task has no unresolved \`blockedBy\` dependencies. Do NOT start work on a task claimed by another agent.
