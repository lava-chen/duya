...
import { serializeSnapshot, applySnapshot } from '../persistence.js';
...
describe('GoalTracker — pause / resume / auto-pause', () => {
  ...
  it('budget_limit → budget_limited terminal; only clear or fresh start exits', () => {
    const t = new GoalTracker();
    t.transition({ type: 'start', objective: 'X' });
    ...
    // no resume from budget_limited
    expect(t.transition({ type: 'resume' })).toBe(false);
    expect(t.state()).toBe('budget_limited');

    // fresh start works (restart with a new budget)
    expect(t.transition({ type: 'start', objective: 'Y', budget: 100 })).toBe(true);
    expect(t.state()).toBe('active');
...
describe('GoalTracker — snapshot / restore', () => {
  it('round-trips an active goal through serializeSnapshot → applySnapshot', () => {
    const t = new GoalTracker();
  ...
  it('folds verifying → active on restore', () => {
    const t = new GoalTracker();
    t.transition({ type: 'start', objective: 'X' });
    t.transition({ type: 'report_completed' });
    expect(t.state()).toBe('verifying');
