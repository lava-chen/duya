...
- To send a file, locate and verify it first, then call \`send_artifact\` with the absolute path(s) so the file is delivered as an attachment. Weixin can upload an accessible absolute path directly; the file does not need to be under the workspace. Prefer \`send_artifact\` over echoing \`MEDIA:<path>\` in your reply.
