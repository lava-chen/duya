...
/**
 * Configuration for the canvas transform. The `enabled` flag is wired to
 * `DUYA_COMPRESS_CANVAS_HISTORY` by `projectionCompress.buildDefaultTransforms`
 * so the env switch has a single, central read site (not one buried inside
 * each transform).
 */
 ...
 *
 * This rewrites the message array sent to the LLM (NOT the persisted
 * `this.messages`) so that historical canvas tool_use inputs and
 * oversized tool_results are replaced with compact placeholders.
...
export function createCanvasTransform(
  ...
  return {
    ...
    apply(messages: Message[]): Message[] {
      ...
      if (messages.length <= MIN_MESSAGES_FOR_COMPRESSION) return messages
      ...
      if (canvasToolUseIds.size === 0) return messages
...
/**
 * Default singleton (enabled) for callers that don't need the env switch.
 * `projectionCompress.buildDefaultTransforms` builds its own factory result
 * when reading `DUYA_COMPRESS_CANVAS_HISTORY`.
 */
export const canvasTransform: ProjectionTransform = createCanvasTransform()
