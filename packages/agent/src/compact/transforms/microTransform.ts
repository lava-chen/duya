...
/**
 * Micro-compact cleanup for tool results. Replaces the result bodies of
 * non-Read tool calls outside the recent-N window with a fixed placeholder.
 *
 * Returns a new message array only when at least one block was rewritten;
 * otherwise the input reference is returned unchanged. Pure — never mutates
 * the input or its messages.
 */
export const microTransform: ProjectionTransform = {
  name: 'micro-compact',
