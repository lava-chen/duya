/**
 * Team memory prompts.
 * Adapted from claude-code-haha memdir/teamMemPrompts.ts
 */
...
export function buildCombinedMemoryPrompt(
  ...
  const howToSave = skipIndex
    ? [
        ...
        '',
        "Write each memory to its own file in the chosen directory (private or team) using this frontmatter format:",
        '',
        ...MEMORY_FRONTMATTER_EXAMPLE,
        '',
    ...
    : [
        ...
        ...MEMORY_FRONTMATTER_EXAMPLE,
        '',
        `**Step 2** — add a pointer to that file in the same directory's \`${ENTRYPOINT_NAME}\`. Each directory (private and team) has its own \`${ENTRYPOINT_NAME}\` index — each entry should be one line, under ~150 characters: \`- [Title](file.md) — one-line hook\`.`,
        '',
  ...
  const lines = [
    '# Memory',
    '',
    `You have a persistent, file-based memory system with two directories: a private directory at \`${autoDir}\` and a shared team directory at \`${teamDir}\`. ${DIR_EXISTS_GUIDANCE}`,
    '',
