...
export async function* withRetry<T extends SSEEvent>(
  ...
  while (state.attempt <= fullConfig.maxRetries) {
    ...
    if (fullConfig.signal?.aborted) {
      DEBUG && console.debug('[duya-ai]', 'Abort signal received, stopping retries');
    ...
    try {
      DEBUG && console.debug(`[duya-ai] Attempt ${state.attempt}/${fullConfig.maxRetries + 1}`);
      ...
      DEBUG && console.debug(`[duya-ai] Operation succeeded on attempt ${state.attempt}`);
      return;
      ...
      if (isAbortError(error)) {
        DEBUG && console.debug('[duya-ai] Abort error, not retrying');
        throw error;
      }
      ...
      if (hasYieldedContent) {
        DEBUG && console.debug('[duya-ai] Already yielded content, not retrying to avoid duplicates');
        throw error;
      }
      ...
      if (!llmError.isRetryable) {
        DEBUG && console.debug('[duya-ai] Error is not retryable, throwing');
        throw llmError;
      }
      ...
      if (state.attempt > fullConfig.maxRetries && !shouldContinueInPersistentMode) {
        DEBUG && console.debug('[duya-ai] Max retries exceeded, throwing last error');
        throw llmError;
      }
      ...
      // Log retry attempt
      DEBUG && console.info(
        `[duya-ai] Retrying after ${delayMs}ms`,
      ...
      if (persistentMode && delayMs > fullConfig.heartbeatIntervalMs) {
        ...
        await sleepWithHeartbeat(
          ...
          (remaining) => {
            ...
            DEBUG && console.debug(`[duya-ai] Heartbeat: ${remaining}ms remaining`);
          },
          fullConfig.signal
        );
      } else {
