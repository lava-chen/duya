...
>
> **既有缺口（非 475 引入）→ ✅ 已修（2026-09-03，同日跟进 commit）**：手动 `/compact` 路径（agent-process-entry.ts，`agent.compact()` 不触发 `onMessagesCompacted`）仍走 legacy `appendMessages`-of-all（Plan 441 头注释已声明 deprecated），**不发 rebase** → 被压缩消息在 rollout 中永不被 supersede，重载后幽灵历史复活（旧消息与 summary 行并存）。修法：`agent.compact()` 成功路径补发 `this.onMessagesCompacted?.(this.messages.length)`（与 streamChat proactive 路径对称），worker 既有 journal 接线自动发 rebase；手动处理器删除 legacy `appendMessages` 调用。单测 3 项（成功触发一次且计数正确 / strategy none 不触发 / 空时间线抛错不触发），3/3 绿。
