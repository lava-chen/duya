...
const WINDOWS_BASH_SHIM = (bundle: string): string =>
`#!/usr/bin/env sh
# DUYA CLI wrapper for Unix-style shells on Windows (Git Bash, MSYS2, MinGW).
# Forwarded to the .cmd wrapper so behavior matches cmd.exe exactly.
# Generated by DUYA CLI installer; do not edit.
exec cmd //c duya "$@"
`;
...
export async function writeWrapperScript(
  ...
  if (platform === 'win32') {
    ...
    // Bash shim (no extension) for Git Bash / MSYS2 / MinGW. POSIX
    // shells see the shebang and exec this directly; Windows shells
    // (cmd, PowerShell, Explorer) ignore the extensionless file and
    // continue using duya.cmd via PATHEXT.
    const bashShim = paths.wrapper.replace(/duya\.cmd$/, 'duya');
    await fsp.writeFile(
      bashShim,
      WINDOWS_BASH_SHIM(paths.bundle),
...
export async function uninstallCli(): Promise<InstallResult> {
  ...
  try {
    ...
    if (platform === 'win32') {
      ...
      const bashShim = wrapper.replace(/duya\.cmd$/, 'duya');
      if (fs.existsSync(bashShim)) await fsp.unlink(bashShim);
    }
