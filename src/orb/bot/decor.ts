 ...
 *
 * Duya change: the source material cycles the whole colour wheel. The orb is
 * branded, so every hue is folded onto the violet band below instead — the
 * rings and comet ribbons stay purple while keeping the measured S/L and the
 * same gradient-along-the-path structure.
 */
const DUYA_HUE_MIN = 250
const DUYA_HUE_SPAN = 45

function wheel(hue: number, s = 0.55, l = 0.62): string {
  const h = DUYA_HUE_MIN + ((((hue % 360) + 360) % 360) / 360) * DUYA_HUE_SPAN
...
/** Violet duya (le bleu d'origine, reteinte pour la marque). */
export const NOTIF_VIOLET = '#a855f7'
